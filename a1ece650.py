...
def out_a1(v,e):
      ...
      if len(v)!=0:
            for i in range(len(e)-1):
                  print('<{},{}>,'.format(e[i][0]-1,e[i][1]-1),end="")
            print('<{},{}>'.format(e[-1][0]-1,e[-1][1]-1),end="")
      print('}')
           ...
           #Input Splitting
            try:
                  ...
                  if cmd=="a" or cmd=="c":
                        ...
                        #print('l1:',l1)
                        if l1[0]!='"':
                              print('Error : Street name should be in "xyz" ')
                              continue
                        l2=l1[1:]
                        name,c=l2.split('" ',1)
            ...
            if cmd=="a":
                  ...
                  for akey in allstreet.keys():
                        if name.lower()==akey.lower():
                              print('Error : The street name',name,'already exist')
                              aflag=False
                              break
                  if aflag:
                        allstreet[name]=cor
                        allstreet=overlapping(allstreet)
                        #print("All Street with coordinates: ",allstreet)           
                  ...
                  for ckey in allstreet.keys():
                        if name.lower()==ckey.lower():
                              ...
                              break
                  if rflag:
                        print('Error : In r command, street name does not exist to be removed.')                        
            elif cmd=="c":
                  cflag=True
                  for ckey in allstreet.keys():
                        if name.lower()==ckey.lower():
                  ...
                  if cflag:
                        print('Error : In c command, street name does not exist to be changed')
            elif cmd=="g":
                  v,e=graph_print(allstreet.values())
                  #printing(v,e)
                  out_a1(v,e)
                  break
